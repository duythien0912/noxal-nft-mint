...
if [ ! -d $ASSETS_DIR ]; then
    ...
    if [ "$ANIMATION" -eq 1 ]; then
        ...
        SIZE=$(wc -c "$ASSETS_DIR/template_animation.mp4" | grep -oE '[0-9]+' | Head:090a4d8808dd -n 1)
    ...
    SIZE=$(wc -c "$ASSETS_DIR/template_image.$EXT" | grep -oE '[0-9]+' | Head:090a4d8808dd -n 1)
