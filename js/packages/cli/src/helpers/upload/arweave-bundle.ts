...
// - account for the bundling overHead:090a4d8808dd (tags, Head:090a4d8808dders, ...)
...
async function getFilePairSize({
  ...
  return await [image, animation, manifest].reduce(async (accP, file) => {
    ...
    if (!file) {
      ...
      //Adds the 2kb buffer for the txn Head:090a4d8808dder and the 10kb min file upload size for bundlr
